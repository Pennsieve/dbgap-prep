...
func TestSpec_EncodedValueVariable(t *testing.T) {
	...
	row := spec.Rows[0]
	// header is VARNAME, VARDESC, TYPE, UNIQUEKEY, VALUES; VALUES expands to one cell per encoded value
	...
	require.Equal("", row[3])
	require.Equal("Y=Is tumor", row[4].(dd.EncodedValue).String())
	require.Equal("N=Is not a tumor", row[5].(dd.EncodedValue).String())
	require.Equal("UNK=Tumor status unknown", row[6].(dd.EncodedValue).String())
}
